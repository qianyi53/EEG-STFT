import os
import time
from addition import R_squared
from addition import bcolors
import numpy as np
import scipy.io
...
if os.path.isdir('logs/' + subfolder + '/'+time_str)==False:
    ...
    os.mkdir('logs/' + subfolder + '/'+time_str+'/checkpoint_dir')
...
#-------------------------分别训练每一个数据集——————————————————————
R2_list=np.empty([108,5])
time.clock()
for m in range(12):
    for n in range(9):
        print(bcolors.OKGREEN + 'processing HS_P'+str(m+1)+'_S'+str(n+1) + bcolors.ENDC)
        # 读取数据
        mat = mat = scipy.io.loadmat('data/100HZ/HS_P'+str(m+1)+'_S'+str(n+1)+'.mat')
        EMG = mat['EMG']
        EEG = mat['EEG']
        sample_num = EEG.shape[0]
        EEG_ch_num = EEG.shape[1]
        ...
        with tf.Session(graph=graph, config=config) as sess:
            ...
            saver.save(sess,"logs/"+subfolder+'/'+ time_str +'/checkpoint_dir/HS_P'+str(m+1)+'_S'+str(n+1))
            ...
            R2_list[m*9+n,:]=r2_val
            #保存结果
            scipy.io.savemat('results/'+subfolder+'/'+time_str+'/'+'HS_P'+str(m+1)+'_S'+str(n+1)+'.mat',{'actual':actual,'pred':pred})

            print(bcolors.OKGREEN + 'finished HS_P' + str(m + 1) + '_S' + str(n + 1) +' time '+str(time.clock())+'s'+ bcolors.ENDC)
summary_writer.close()
print(np.array2string(np.array(R2_list),precision=2))
scipy.io.savemat('results/'+subfolder+'/'+time_str+'/R2.mat',{'R2':R2_list})
